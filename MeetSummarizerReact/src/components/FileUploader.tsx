...
export const FileUploader = () => {
    ...
    useEffect(() => {
        if (meetingId) {
            dispatch(fetchMeetingsByTeam({ teamId: Number(meetingId) }));
        }
    }, [dispatch, meetingId]);
    ...
    const handleUpload = async () => {
        ...
        if (!meetingId || !meeting?.teamId) {
            alert("❌ לא נמצא meetingId או teamId.");
            return;
        }
    ...
    return (
        <div>
            <h2>📂 העלאת קובץ לישיבה</h2>
            <input type="file" onChange={handleFileChange} />
            <button onClick={handleUpload} disabled={!file || !meeting}>📤 העלה קובץ</button>
            {progress > 0 && <div>🔄 התקדמות: {progress}%</div>}
            {uploadUrl && <div>📂 קובץ הועלה ל-S3: <a href={uploadUrl} target="_blank" rel="noopener noreferrer">🔗 פתח</a></div>}
            {downloadUrl && <button onClick={downloadFileToComputer}>📥 הורד קובץ</button>}
        </div>
